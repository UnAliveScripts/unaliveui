...
-- Main UI container (hidden initially, shown after key validation)
local mainUIContainer = Instance.new("Frame")
mainUIContainer.Name = "MainUI"
mainUIContainer.Size = UDim2.fromScale(1, 1)
mainUIContainer.BackgroundTransparency = 1
mainUIContainer.BorderSizePixel = 0
mainUIContainer.ZIndex = 20
mainUIContainer.Visible = false
mainUIContainer.Parent = card

-- Search bar (top-right)
local sf = Instance.new("Frame", mainUIContainer); sf.Name = "SearchField"
...
sfBox.Focused:Connect(function() sfPH.Visible = false; TweenService:Create(sfBorder, TweenInfo.new(0.2), {Color = Color3.fromRGB(200,200,200), Transparency = 0.5}):Play() end)
sfBox.FocusLost:Connect(function() syncPH(); TweenService:Create(sfBorder, TweenInfo.new(0.2), {Color = white, Transparency = 0.92}):Play() end)
sfClear.MouseButton1Click:Connect(function() sfBox.Text = ""; sfBox:CaptureFocus() end)
syncPH()

-- EditMenu (bottom of card)
local TS2 = TweenService
local em = Instance.new("Frame", mainUIContainer); em.Name = "EditMenu"
...
-- Pages container
local pages = Instance.new("Frame", mainUIContainer); pages.Name = "Pages"
...
for ei, ed in ipairs(items) do
	...
	hb.MouseButton1Click:Connect(function()
		...
		if selectedLabel then TS2:Create(selectedLabel, TweenInfo.new(0.2, Enum.EasingStyle.Quad), { TextColor3 = normalColor }):Play() end
		TS2:Create(lb, TweenInfo.new(0.2, Enum.EasingStyle.Quad), { TextColor3 = selectedColor }):Play()
		selectedLabel = lb
		-- Switch page
...
btn.MouseButton1Click:Connect(function()
	...
	-- Hide key elements, show main UI
	card:FindFirstChild("Heading").Visible = false
	card:FindFirstChild("Subtitle").Visible = false
	card:FindFirstChild("Enter Key_Field").Visible = false
	card:FindFirstChild("LoginBtn").Visible = false
	card:FindFirstChild("GetKeyLink").Visible = false

	local mainUI = card:FindFirstChild("MainUI")
	if mainUI then
		mainUI.Visible = true
		print("[UnAlive] ✓ Main UI unlocked")
	else
		-- Fallback: load demo
		local UI_URL = "https://raw.githubusercontent.com/UnAliveScripts/unaliveui/main/"
		local ok, r = pcall(function() return loadstring(game:HttpGet(UI_URL .. "demo.lua"))() end)
		if ok then print("[UnAlive] ✓ Main UI loaded") else warn("[UnAlive] Failed to load:", r) end
	end
end)
