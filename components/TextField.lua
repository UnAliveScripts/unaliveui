...
return function(self, props)
    ...
    box.FocusLost:Connect(function() TweenService:Create(container.__instance, TweenInfo.new(0.2, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), { BackgroundColor3 = Color3.fromRGB(44, 44, 50) }):Play(); if props.ValueChanged then props.Value = box.Text; task.spawn(props.ValueChanged, obj, box.Text) end end)
    local obj = { Type = "TextField", Theme = self and self.Theme, __instance = container.__instance }
    function obj.Parent(p) container.Parent = p end
    function obj:SetValue(v) box.Text = v end
    function obj:SetPlaceholder(v) box.PlaceholderText = v end
    return obj
end