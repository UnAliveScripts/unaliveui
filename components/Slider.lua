...
return function(self, props)
    ...
    local thumb = create("ImageButton")({ Name = "Thumb", BackgroundColor3 = Color3.fromRGB(255, 255, 255), BorderSizePixel = 0, AutoButtonColor = false, Position = UDim2.fromOffset(0, 2), Size = UDim2.fromOffset(20, 20), Image = "rbxassetid://109321922977357", ImageTransparency = 0.2, ZIndex = 2, create("UICorner")({ CornerRadius = UDim.new(1, 0) }) })
    ...
    local function setPosition(val)
        ...
        local w = container.AbsoluteSize.X - 20
        local x = math.floor(pct * w)
        s.Thumb.Position = UDim2.fromOffset(x, 2)
        s.Fill.Size = UDim2.fromOffset(x + 10, 4)
    end
    
    local dragging = false
    s.Thumb.InputBegan:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.MouseButton1 then dragging = true end
    end)
    s.Thumb.InputEnded:Connect(function(i)
