...
return function(self, props)
    ...
    local theme = self.Theme.Controls.Toggle; local parent = self.__container or self.__instance or self; local struct = {}

    struct.Body = C("Frame")({ Name = "Toggle", BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.fromOffset(28,16), Parent = parent,
        C("ImageButton")({ Name = "Switch", AutoButtonColor = false, BackgroundTransparency = 1, BorderSizePixel = 0,
            ...
            __dynamicKeys = { ImageColor3 = theme.SwitchOff[1], ImageTransparency = theme.SwitchOff[2] },
            C("ImageLabel")({ Name = "Knob", AnchorPoint = Vector2.new(0,0.5), BackgroundTransparency = 1, BorderSizePixel = 0,
                ...
                __dynamicKeys = { ImageColor3 = theme.Knob[1], ImageTransparency = theme.Knob[2] },
                C("ImageLabel")({ Name = "FX", AnchorPoint = Vector2.new(0.5,0.5), BackgroundTransparency = 1, BorderSizePixel = 0,
                    ...
                    __dynamicKeys = { ImageColor3 = theme.KnobEffects[1], ImageTransparency = theme.KnobEffects[2] } }) }),
            C("UIGradient")({ Name = "Depth", Rotation = 90, __dynamicKeys = { Color = theme.DepthEffect } }) }),
    })

    local sw = struct.Body.__instance:FindFirstChild("Switch"); local knob = sw:FindFirstChild("Knob")
    ...
    local function update(v, inst)
        ...
        if inst then knob.Position = kp.Position; sw.ImageColor3 = sc; sw.ImageTransparency = st; return end
        TS:Create(knob, ti, kp):Play(); TS:Create(sw, ti, { ImageColor3 = sc, ImageTransparency = st }):Play()
    end
    update(props.Value, true)

    sw.MouseButton1Click:Connect(function() props.Value = not props.Value; update(props.Value); if props.ValueChanged then task.spawn(props.ValueChanged, obj, props.Value) end end)

    local obj = { Type = "Toggle", Theme = self.Theme, Structures = struct, __instance = struct.Body }
