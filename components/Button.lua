...
return function(self, props)
	...
	struct.Body = C("TextButton")({
		...
		C("Frame")({
			...
			C("UIStroke")({
				...
				Transparency = 0.95,
				__dynamicKeys = { Color = theme.Shadow },
			}),
		}),
	...
	struct.Body.MouseButton1Down:Connect(function()
		TS:Create(struct.Overlay, TweenInfo.new(0.1, Enum.EasingStyle.Cubic), {
			BackgroundTransparency = 0.8,
		}):Play()
	end)

	struct.Body.MouseButton1Up:Connect(function()
		TS:Create(struct.Overlay, TweenInfo.new(0.2, Enum.EasingStyle.Sine), {
			BackgroundTransparency = 1,
		}):Play()
	end)

	struct.Body.MouseLeave:Connect(function()
		TS:Create(struct.Overlay, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {
			BackgroundTransparency = 1,
		}):Play()
	end)

	struct.Body.MouseButton1Click:Connect(function()
		if props.Pushed then
			task.spawn(props.Pushed, obj)
		end
	end)
